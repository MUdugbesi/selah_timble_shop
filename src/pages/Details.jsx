...
const Details = () => {
    ...
    return (
        <div className={`${!statusTab ? 'opacity-100' : 'opacity-50'}`}>
            ...
            {detail.name ? <div className='w-[80%] lg:w-[60%] mx-auto h-auto grid grid-cols-1 md:grid-cols-2 mb-[20px] bg-white p-10'>
                <div className='flex justify-between items-center mr-3 w-[90%]'>
                    {num > 0 ? <MdKeyboardArrowLeft className='size-6 hover:cursor-pointer active:text-[red]' onClick={handleLeftArrow} /> : ''}
                    <img src={`https://api.timbu.cloud/images/${detail.photos[num].url}`} className='w-[85%] h-[389px] mx-auto object-contain object-center bg-transparent' />
