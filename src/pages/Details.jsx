...
const Details = () => {
    ...
    const handleRightArrow = () => {
        if (detail.photos && num < detail.photos.length - 1) {
            setNum(prev => prev + 1);
        } else {
            setNum(0);
        }
    ...
    return (
        <div className={`${!statusTab ? 'opacity-100' : 'opacity-50'} h-auto md:min-h-[20vh] pb-[50px] md:mt-[50px]`}>
            ...
            <div className='w-[90%] md:[80%] lg:w-[60%] mx-auto min-h-auto grid grid-cols-1 md:grid-cols-2 mb-[20px] bg-white p-10'>
                ...
                <div className='p-2'>
                    ...
                    <div className='flex w-full h-[40px] justify-start items-center relative mt-10'>
                        <span className='opacity-50 font-lato font-light text-[14px] mr-6'>
                            {!toggleFav ? <MdOutlineFavoriteBorder className='size-5 hover:cursor-pointer' onClick={handleFav} /> : <MdFavorite className='size-5 hover:cursor-pointer text-[red]' onClick={handleFav} />}
                        </span>
                    ...
                    <div className='w-[80%] lg:w-[50%] h-auto flex gap-[2px] mt-[50px]'>
                        ...
                        <button className='flex md:w-[60%] h-[35px] font-lato mx-auto gap-3 justify-center items-center text-button p-2 border border-text rounded-r-sm text-sm hover:cursor-pointer hover:opacity-80 active:text-red-600' onClick={handleAddToCart}>
                            <FaCartPlus size={18} className='text-button' />Add to Cart
                        </button>
                    </div>
                </div>
            </div>
